angular
  ...
  .service(
    ...
    function (
      ...
      var optionalValues;
      $http.get("artifacts/options.json").then(function (data) {
      ...
      function setUpReferenceResources(fhirVersion) {
        ...
        if (fhirVersion == 3) {
          referenceResources.push({
            resourceType: "Practitioner",
            name: [{ text: "Dr John Doe" }],
            identifier: [{ value: "jd", system: identifierSystem }],
            text: {
              status: "generated",
              div: "<div xmlns='http://www.w3.org/1999/xhtml'>Dr John Doe</div>",
            },
          });
          referenceResources.push({
            resourceType: "Practitioner",
            name: [{ text: "Dr Annette Jones" }],
            identifier: [{ value: "aj", system: identifierSystem }],
            text: {
              status: "generated",
              div: "<div xmlns='http://www.w3.org/1999/xhtml'>Dr Annette Jones</div>",
            },
          });
          referenceResources.push({
          ...
          referenceResources.push({
            ...
            name: [{ text: "Dr John Doe" }],
            identifier: [{ value: "jd", system: identifierSystem }],
            text: { status: "generated", div: "<div>Dr John Doe</div>" },
          });
          referenceResources.push({
            resourceType: "Practitioner",
            name: [{ text: "Dr Annette Jones" }],
            identifier: [{ value: "aj", system: identifierSystem }],
            text: { status: "generated", div: "<div>Dr Annette Jones</div>" },
          });
          referenceResources.push({
            resourceType: "Organization",
            name: "clinFHIR Sample creator",
            identifier: [{ value: "cf", system: identifierSystem }],
            text: { status: "generated", div: "<div>clinFhir</div>" },
          });
        }
      }
      ...
      return {
        ...
        createPatient: function (input, cfOrganization) {
          ...
          if (input.identifier) {
            ...
            patient.identifier = [
              {
                system: appConfigSvc.config().standardSystem.identifierSystem,
          ...
          if (cfOrganization) {
            patient.managingOrganization = {
              display: "sampleBuilder",
              reference: "Organization/" + cfOrganization.id,
            }; //<<<< todo make a real org... - check at startus
          }
          ...
          $http.post(uri, patient, serverInteractionSvc.getServerConfig()).then(
            function (data) {
              ...
              if (id) {
                ...
                deferred.reject({
                  err: "The server did not return a valid id",
                });
              }
            },
            function (err) {
        ...
        createAppointments: function (patientId, options) {
          ...
          var data = [
            {
              ...
              who: { text: "Clarence cardiology clinic" },
              delay: 4,
            },
            {
              status: "pending",
          ...
          data.forEach(function (item, index) {
            ...
            appt.participant = [
              { actor: { display: item.who.text }, status: "accepted" },
            ]; //the perfromed
            var txt =
        ...
        createObservations: function (patientId, options) {
          ...
          var cnt = 0;
          for (var i = 0; i < 6; i++) {
            //5 sets per type of observation
            var date = moment().subtract(i, "weeks");

            //decide if this set of observations will be linked to an encounter...
            var encounter = this.getRandomReferenceResource(
              "Encounter",
              buildConfig.encounterObservation,
            ); //half the time it will be null
            ...
            observations.forEach(function (item, j) {
              ...
              obs.code = {
                text: item.display,
                coding: [{ system: "http://loinc.org", code: item.code }],
              };
        ...
        buildAllergiesList: function (patientId, options) {
          ...
          var allergyList = {
            ...
            text: {
              status: "generated",
              div: "<div xmlns='http://www.w3.org/1999/xhtml'>Allergies list</div>",
            },
            code: {
              coding: [
                {
          ...
          GetDataFromServer.localServerQuery(url).then(
            function (data) {
              ...
              refList.forEach(function (allergy, inx) {
                ...
                var practitioner =
                  that.getRandomReferenceResource("Practitioner"); //there will always be a practitioner
                allergy.recorder = {
                ...
                var entry = {
                  date: today,
                  item: { reference: "AllergyIntolerance/" + allergy.id },
                };
              ...
              $http
                .post(url, bundle, serverInteractionSvc.getServerConfig())
                .then(
                  function (data) {
                    if (options.logFn) {
            ...
            function (err) {
              alert(
                "error getting list of allergies to create: " +
                  angular.toJson(err),
              );
            },
          );
        ...
        buildConditionList: function (patientId, options) {
          ...
          var conditionList = {
            ...
            text: {
              status: "generated",
              div: "<div xmlns='http://www.w3.org/1999/xhtml'>Problem list</div>",
            },
            code: {
              coding: [
                {
          ...
          GetDataFromServer.localServerQuery(url).then(
            function (data) {
              ...
              refList.forEach(function (condition, inx) {
                ...
                var entry = {
                  date: today,
                  item: { reference: "Condition/" + condition.id },
                };
            ...
            function (err) {
              alert(
                "error getting list of conditions to create: " +
                  angular.toJson(err),
              );
            },
          );
        ...
        buildMedicationList: function (patientId, options) {
          ...
          var medList = {
            ...
            text: {
              status: "generated",
              div: "<div xmlns='http://www.w3.org/1999/xhtml'>Medication list</div>",
            },
            code: {
              coding: [
                {
          ...
          GetDataFromServer.localServerQuery(url).then(
            function (data) {
              ...
              refList.forEach(function (medStmt, inx) {
                ...
                var entry = {
                  date: today,
                  item: { reference: "MedicationStatement/" + medStmt.id },
                };
            ...
            function (err) {
              alert(
                "error getting list of medications to create: " +
                  angular.toJson(err),
              );
            },
          );
        ...
        createEncounters: function (patientId, options, bundleConditions) {
          ...
          for (var i = 0; i < options.count; i++) {
            ...
            for (var j = 0; j < cnt; j++) {
              var ref =
                bundleConditions.entry[
                  parseInt(bundleConditions.entry.length * Math.random())
                ];
              //console.log(ref)
              if (
                ref.response.location &&
                ref.response.location.indexOf("Condition") > -1
              ) {
                if (fhirVersion == 2) {
                  enc.indication.push({ reference: ref.response.location });
                } else {
                  enc.diagnosis.push({
                    condition: { reference: ref.response.location },
                  });
                }
              }
            }
            ...
            enc.period = {
              start: moment().subtract(
                parseInt(options.period * Math.random()),
                "days",
              ),
            };

            var txt =
              moment(enc.period.start).format("MMM Do YYYY") +
              " for " +
              enc.reason[0].text;
            enc.text = {
              status: "generated",
              div:
                "<div xmlns='http://www.w3.org/1999/xhtml'>" + txt + "</div>",
            };

            var practitioner = this.getRandomReferenceResource("Practitioner");
            enc.participant = [];
            enc.participant.push({
              individual: {
                reference: "Practitioner/" + practitioner.id,
                display: practitioner.name.text,
              },
            }); //safe 'cause I created the practitioner...
        ...
        checkReferenceResources: function () {
          ...
          referenceResources.forEach(function (res) {
            ...
            arQuery.push(
              checkAndInsert(res).then(function () {
                //maybe not neded
              }),
            );
          });
          ...
          function checkAndInsert(res) {
            ...
            var identifierQuery = encodeURIComponent(
              res.identifier[0].system + "|" + res.identifier[0].value,
            );
            ...
            $http.get(url, serverInteractionSvc.getServerConfig()).then(
              ...
              function (err) {
                console.log(err);
                alert(
                  "There was an error accessing the server. It may not be set up for CORS, " +
        ...
        getVitals: function (vo) {
          ...
          observations.forEach(function (item) {
            if (item.isVital) {
              ...
              response.vitalsCodes.push({
                code: item.code,
                display: item.display,
                unit: item.unit,
              }); //the list of codes used by the display
            }
          });
        ...
        getAllResourcesFollowingPaging: function (
          url,
          limit,
          oauthAccessToken,
          apiKey,
...
*/
        ...
        getAllData: function (patientId) {
          ...
          if (dataServer.everythingOperation || dataServer.everything) {
            ...
            this.getAllResourcesFollowingPaging(
              url,
              null,
              null,
              dataServer.apiKey,
              ...
              function (arrayOfResource) {
                ...
                if (arrayOfResource) {
                  arrayOfResource.forEach(function (resource) {
                    //this is a bundle
                ...
                function getMedName(m) {
                  return ResourceUtilsSvc.getOneLineSummaryOfResource(
                    m.resource,
                  );
                }
              },
          ...
          resources.forEach(function (item) {
            ...
            arQuery.push(
              getAllResources(uri).then(function (bundle) {
                if (
          ...
          $q.all(arQuery).then(
            ...
            function (err) {
              alert(
                "error loading all patient data:\n\n" + angular.toJson(err),
              );
              deferred.reject(err);
            },
          );
                ...
                if (referenceResources) {
                  data.data.entry.forEach(function (entry, index) {
                    ...
                    if (resource && resource.id) {
                      ...
                      deferred.reject(
                        "a resource was added, but no id was supplied",
                      );
                    }
