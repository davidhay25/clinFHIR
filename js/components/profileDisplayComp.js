  ...
  controller: function (
    ...
    this.$onChanges = function (obj) {
      ...
      if (obj.restoreatpath) {
        ...
        if (
          that.baseSD &&
          that.baseSD.snapshot &&
          that.baseSD.snapshot.element
        ) {
          ...
          if (edToRestore) {
            ...
            for (
              var i = 0;
              i < that.selectedProfile.snapshot.element.length;
              i++
            ) {
              var edProfile = that.selectedProfile.snapshot.element[i];
              if (edProfile.path == root) {
                that.selectedProfile.snapshot.element.splice(
                  i + 1,
                  0,
                  edToRestore,
                );
          ...
          if (ed.path == edToRestore.path) {
            if (ed.path.indexOf("xtension") > -1) {
              ...
              if (edToRestore.type) {
                ...
                urlOfTypeToRestore = Utilities.getProfileFromType(
                  edToRestore.type[0],
                );

                if (ed.type) {
                  if (
                    Utilities.getProfileFromType(ed.type[0]) ==
                    urlOfTypeToRestore
                  ) {
        ...
        that.selectedProfile.snapshot.element.forEach(function (ed) {
          if (ed.path == edToDelete.path) {
            ...
            if (ed.path.indexOf("xtension") > -1) {
              ...
              var urlOfTypeToDelete;
              if (edToDelete.type) {
                urlOfTypeToDelete = Utilities.getProfileFromType(
                  edToDelete.type[0],
                );
      ...
      if (obj.profile && obj.profile.currentValue) {
        ...
        if (that.selectedProfile) {
          ...
          var baseDefinition =
            that.selectedProfile.baseDefinition || that.selectedProfile.base;
      ...
      var modalInstance = $uibModal.open({
        ...
        controller: function ($scope, uri) {
          ...
          $scope.showAll = function () {
            $scope.waiting = true;
            GetDataFromServer.getExpandedValueSet($scope.vs.id)
              .then(
                function (result) {
          ...
          $scope.search = function (filter) {
            $scope.waiting = true;
            GetDataFromServer.getFilteredValueSet($scope.vs.id, filter)
              .then(
                function (result) {
                  if (result.expansion) {
                    $scope.data = result.expansion.contains;
          ...
          GetDataFromServer.getValueSet(uri)
            .then(function (vs) {
              $scope.vs = vs;
              console.log(vs);
            })
    ...
    this.getTable = function (treeDivId) {
      ...
      if (this.selectedProfile) {
        ...
        profileCreatorSvc
          .makeProfileDisplayFromProfile(that.selectedProfile)
          .then(function (data) {
            that.buildView = data;
