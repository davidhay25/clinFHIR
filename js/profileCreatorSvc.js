  ...
  .service(
    ...
    function (
      ...
      function makeExtensionSD(vo) {
        ...
        extensionSD.identifier = [
          { system: "http://clinfhir.com", value: "author" },
        ];
      ...
      return {
        ...
        makeProfileDisplayFromProfile: function (inProfile) {
          ...
          if (profile && profile.snapshot && profile.snapshot.element) {
            profile.snapshot.element.forEach(function (item, inx) {
              ...
              if (!path) {
                alert(
                  "empty path in Element Definition\n" + angular.toJson(item),
                );
                return;
              }
                    ...
                    if (it.code == "Extension" && it.profile) {
                      ...
                      queries.push(
                        GetDataFromServer.findConformanceResourceByUri(
                          ...
                          function (sdef) {
                            var analysis =
                              Utilities.analyseExtensionDefinition3(sdef);
                          ...
                          function (err) {
                            modalService.showModal(
                              ...
                              {
                                bodyText:
                                  "makeProfileDisplayFromProfile: Error retrieving " +
                                  it.profile +
                                  " " +
                                  angular.toJson(err),
                              },
              ...
              if (ar.length == 2 && elementsToDisable.indexOf(ar[1]) > -1) {
                addLog(
                  "excluding " + item.path + " as in list of elementsToDisable",
                );
                include = false;
              }
              ...
              if (item.type) {
                item.type.forEach(function (it) {
                  ...
                  if (
                    ["code", "Coding", "CodeableConcept"].indexOf(it.code) > -1
                  ) {
              ...
              if (include) {
                ...
                if (
                  item.myMeta.isExtension ||
                  (item.builderMeta && item.builderMeta.isExtension)
                ) {
            ...
            lstTree.forEach(function (node) {
              ...
              if (node.text && node.text.indexOf("[x]") == -1) {
                ...
                if (
                  ...
                  node.data.ed &&
                  node.data.ed.myMeta &&
                  node.data.ed.myMeta.analysis &&
                  node.data.ed.myMeta.analysis.dataTypes &&
                  node.data.ed.myMeta.analysis.dataTypes.length > 1
                ) {
                  node.text += "[x]";
                }
              }
        ...
        saveNewProfile: function (profileName, model, baseProfile, isEdit) {
          ...
          if (fhirVersion == 3) {
            ...
            if (
              ...
              sd.baseType =
                "http://hl7.org/fhir/StructureDefinition/" + baseProfile.name;
            } else {
            ...
            if (item.data && item.data.ed) {
              ...
              if (ed.myMeta) {
                if (ed.myMeta.remove) {
                  ...
                  ed.myMeta.isNew ||
                  (ed.myMeta.isExtension && ed.myMeta.isDirty)
                ) {
        ...
        diffFromBase: function (profile, appConfigSvc) {
          ...
          if (baseDefinition && profile.snapshot && profile.snapshot.element) {
            ...
            deferred.reject({
              err: "can't resolve the baseDefinition or some other profile issue",
              profile: profile,
            });
          }
