...
angular
  ...
  .service(
    "SaveDataToServer",
    function ($http, $q, appConfigSvc, serverInteractionSvc) {
      return {
        ...
        addTaskForPractitioner: function (practitioner, options) {
          ...
          task.focus = {
            reference: options.focus.resourceType + "/" + options.focus.id,
          };
          ...
          this.saveResource(
            task,
            appConfigSvc.getCurrentConformanceServer().url,
          ).then(
            function () {
        ...
        saveResource: function (resource, urlBase, user) {
          ...
          if (resource.id) {
            ...
            $http
              .put(qry, resource, serverInteractionSvc.getServerConfig())
              .then(
                function (data) {
            ...
            //console.log(qry)
            $http
              .post(qry, resource, serverInteractionSvc.getServerConfig())
              .then(
                function (data) {
                  deferred.resolve(data);
                },
        ...
        deleteResource: function (server, resource) {
          ...
          var url = server.url + "StructureDefinition/" + resource.id;
          $http
            .delete(url, resource, serverInteractionSvc.getServerConfig())
            .then(
              function (data) {
                deferred.resolve(data);
              },
        ...
        saveValueSetToTerminologyServerById: function (id, valueSet) {
          ...
          return $http.put(
            qry,
            valueSet,
            serverInteractionSvc.getServerConfig(),
          );
        },
    ...
    function (
      ...
      return {
        ...
        getOutputsForModel: function (model, type) {
          ...
          var url =
            appConfigSvc.getCurrentDataServer().url +
            "Task?focus=StructureDefinition/" +
            model.id;
            ...
            function (data) {
              ...
              if (data.data && data.data.entry) {
                ...
                data.data.entry.forEach(function (entry) {
                  ...
                  queries.push(
                    that.getOutputsForTask(task, "QuestionnaireResponse").then(
                      function (lst) {
                        // console.log(lst)
                        if (lst.length > 0) {
                ...
                data.data.entry.forEach(function (entry) {
                  if (focus) {
                    ...
                    if (
                      ...
                      focus == entry.resource.focus.reference
                    ) {
        ...
        getPractitionerByLogin: function (user, details) {
          ...
          //the system used by clinFHIR for practitioners...
          var practitionerSystem =
            appConfigSvc.config().standardSystem.practitionerIdentifierSystem;
          ...
          var url =
            ...
            encodeURIComponent(practitionerSystem + "|" + loginIdentifier);
          ...
          this.adHocFHIRQuery(url).then(
            function (data) {
              ...
              if (data.data && data.data.entry && data.data.entry.length > 0) {
                ...
                pract.identifier = [
                  { system: practitionerSystem, value: loginIdentifier },
                ];
                ...
                var createUrl =
                  ...
                  pract.id;
                $http
                  .put(createUrl, pract, serverInteractionSvc.getServerConfig())
                  .then(
                    function () {
            ...
            function (err) {
              ...
              deferred.reject(
                "error getting practitioner " + angular.toJson(err),
              );
            },
          );
        ...
        getValueSet: function (ref) {
          ...
          Utilities.getValueSetIdFromRegistry(ref, function (resp) {
            if (resp) {
              ...
              deferred.reject(
                "The terminology server does not have a ValueSet with the URL: " +
                  ref +
                  "",
              );
            }
        ...
        getVersionHistory: function (resource) {
          ...
          var qry =
            appConfigSvc.getCurrentDataServerBase() +
        ...
        adHocFHIRQueryFollowingPaging: function (url, accessToken) {
          ...
          var deferred = $q.defer();
          var bundle = Utilities.perfromQueryFollowingPaging(
            url,
            null,
            accessToken,
          ).then(
        ...
        findConformanceResourceByUri: function (
          url,
          serverUrl,
          typeOfConformanceResource,
          hideError,
        ) {
          ...
          typeOfConformanceResource =
            typeOfConformanceResource || "StructureDefinition";

          //default to the conformance server...
          serverUrl =
            serverUrl || appConfigSvc.getCurrentConformanceServer().url;
          ...
          $http.get(qry, serverInteractionSvc.getServerConfig()).then(
            function (data) {
              ...
              if (bundle && bundle.entry && bundle.entry.length > 0) {
                  ...
                  if (arEntry.length == 1) {
                    ...
                    if (!hideError) {
                      ...
                      console.log(
                        "There are " +
                          bundle.entry.length +
                          " StructureDefinitions with the url " +
                          url +
                          "!",
                      );
                    }
                    ...
                    arEntry.forEach(function (ent) {
                      ...
                      ent.resource.meta.lastUpdated =
                        ent.resource.meta.lastUpdated || moment().format(); //if no lastUpdated, set to now...

                      var date =
                        moment(ent.resource.meta.lastUpdated) || moment();
                      ...
                      if (ent) {
                        ...
                        deferred.reject({
                          msg:
                            ...
                            serverUrl +
                            typeOfConformanceResource +
                            " (but not a server error - just not present, although others were)",
                        });
                      }
                    });
                  }
                ...
                deferred.reject({
                  msg:
                    "No matching profile with the canonical url: " +
                    url +
                    " found on " +
                    serverUrl +
                    typeOfConformanceResource +
                    " (but not a server error - just not present)",
                });
              }
            },
            function (err) {
        ...
        getFilteredValueSet: function (id, filter) {
          ...
          var qry =
            appConfigSvc.getCurrentTerminologyServer().url +
            "ValueSet/" +
            id +
            "/$expand?filter=" +
            filter;
        ...
        getAccessAudit: function (evt, min, max) {
          ...
          var afterChange;
          $http
            .get(url, serverInteractionSvc.getServerConfig())
            .success(function (data) {
              ...
              var chart1 = {
                series: [{ name: "Count", data: [], type: "areaspline" }],
                titleX: {
                  text: "Daily Access",
                },
                useHighStocks: true,
  ...
  .service(
    "Utilities",
    function ($http, $q, $localStorage, appConfigSvc, modalService) {
      return {
        ...
        analyseExtensionDefinition: function (extension) {
          ...
          if (extension.snapshot) {
            extension.snapshot.element.forEach(function (element) {
              ...
              if (element.path.indexOf("Extension.value") > -1) {
                ...
                if (dt == "reference" || dt == "[x]") {
                  //eg cgif-guidancerequest
                  ...
                  if (element.type) {
                    element.type.forEach(function (t) {
                      ...
                      if (p) {
                        ...
                        item.specification =
                          "http://hl7.org/fhir/" + ar[ar.length - 1]; //really only works if this is a core resource...
          ...
          if (discriminator) {
            //vo.complex=true;
            vo.complexExtension = that.processComplexExtension(
              extension,
              discriminator,
            );
          }
                ...
                if (element.type) {
                  element.type.forEach(function (typ) {
                    ...
                    if (code) {
                      ...
                      if (
                        ["CodeableConcept", "code", "coding"].indexOf(code) > -1
                      ) {
                        vo.isCoded = true;
                      }
            ...
            SD.snapshot.element.forEach(function (element, inx) {
              if (element.path) {
                ...
                if (arPath.length == 2) {
                  ...
                  if (
                    vo.isComplexExtension &&
                    arPath[1].indexOf("value") > -1
                  ) {
                    include = false;
                  } //THIS is the parent...
                }
        ...
        getValueSetIdFromRegistry: function (uri, cb, noError) {
          ...
          $http.get(qry).then(
              ...
              if (bundle && bundle.entry && bundle.entry.length > 0) {
                if (bundle.entry.length > 1) {
                  var alrt =
                    ...
                    config.servers.terminology +
                    ") has multiple ValueSets with a URL property (in the resource) of " +
                    uri +
                    ". I'll use the first one, but you might want to contact the registry owner and let them know.";
                  resp.error = alrt;
                  if (!noError) {
                ...
                config.log(
                  "resolvedId: " + id,
                  "rbServices:getValueSetIdFromRegistry",
                );
            ...
            function (err) {
              alert(
                "Error contacting terminology server:\n" + angular.toJson(err),
              );
              cb(null);
            },
          );
        ...
        isVSaList: function (uri) {
          ...
          vsLookup = [];
          vsLookup["condition-severity"] = {
            id: "valueset-condition-severity",
            minLength: 1,
            type: "list",
          };
          vsLookup["condition-category"] = {
            id: "valueset-condition-category",
            type: "list",
          };
          vsLookup["condition-certainty"] = {
            id: "valueset-condition-certainty",
            minLength: 1,
            type: "list",
          };
          vsLookup["condition-status"] = {
            id: "condition-status",
            minLength: 1,
            type: "list",
          };
          vsLookup["condition-ver-status"] = {
            id: "condition-ver-status",
            minLength: 1,
            type: "list",
          };

          vsLookup["list-empty-reason"] = {
            id: "valueset-list-empty-reason",
            minLength: 1,
            type: "list",
          };
          vsLookup["list-item-flag"] = {
            id: "valueset-list-item-flag",
            minLength: 1,
            type: "list",
          };
          vsLookup["basic-resource-type"] = {
            id: "valueset-basic-resource-type",
            minLength: 1,
            type: "list",
          };
          ...
          vsLookup["investigation-sets"] = {
            id: "valueset-investigation-sets",
            minLength: 1,
            type: "list",
          };
          vsLookup["observation-interpretation"] = {
            id: "valueset-observation-interpretation",
            minLength: 1,
            type: "list",
          };
          vsLookup["marital-status"] = {
            id: "marital-status",
            minLength: 1,
            type: "list",
          };
          vsLookup["ActPharmacySupplyType"] = {
            id: "v3-vs-ActPharmacySupplyType",
            minLength: 1,
            type: "list",
          };
          vsLookup["Confidentiality"] = {
            id: "v2-0272",
            minLength: 1,
            type: "list",
          };
          vsLookup["composition-status"] = {
            id: "composition-status",
            minLength: 1,
            type: "list",
          };
          vsLookup["observation-status"] = {
            id: "observation-status",
            minLength: 1,
            type: "list",
          };
          vsLookup["observation-category"] = {
            id: "observation-category",
            minLength: 1,
            type: "list",
          };

          vsLookup["administrative-gender"] = {
        ...
        setAuthoredByClinFhir: function (resource) {
          ...
          if (resource) {
            var extensionUrl =
              appConfigSvc.config().standardExtensionUrl.clinFHIRCreated;
            this.addExtensionOnce(resource, extensionUrl, {
              valueBoolean: true,
            });
          }
        },
        isAuthoredByClinFhir: function (profile) {
          ...
          var ext = this.getSingleExtensionValue(
            profile,
            appConfigSvc.config().standardExtensionUrl.clinFHIRCreated,
          );
          ...
          if (!isAuthoredByClinFhir) {
            if (profile.code) {
              profile.code.forEach(function (coding) {
                if (
                  coding.system ==
                    "http://fhir.hl7.org.nz/NamingSystem/application" &&
            ...
            if (profile.identifier) {
              if (angular.isArray(profile.identifier)) {
                profile.identifier.forEach(function (ident) {
                  if (
                    ident.system == "http://clinfhir.com" &&
                    ident.value == "author"
                  ) {
                    isAuthoredByClinFhir = true;
                  }
                ...
                if (
                  profile.identifier.system == "http://clinfhir.com" &&
                  profile.identifier.value == "author"
                ) {
                  isAuthoredByClinFhir = true;
                }
              }
              ...
              function (data) {
                ...
                if (bundle.link) {
                  ...
                  for (var i = 0; i < bundle.link.length; i++) {
                    ...
                    if (lnk && lnk.relation == "next") {
                      // && returnBundle.entry.length < limit) {
                      moreToGet = true;
                      var url = lnk.url;
                      getPage(url);
                      break;
  ...
  .service(
    "RenderProfileSvc",
    function ($http, $q, Utilities, ResourceUtilsSvc) {
      ...
      var iterateOverNodeSet = function (insertionPoint, invalue) {
        ...
        angular.forEach(invalue, function (value, propertyName) {
              ...
              if (parentPropertyName == "extension") {
                ...
                if (vo.isPrimitive) {
                  if (!insertionPoint["_" + parentPropertyName]) {
                    insertionPoint["_" + parentPropertyName] = {
                      extension: [],
                    };
                  }
      ...
      return {
        getAllStandardResourceTypes: function () {
          ...
          if (!standardResourceTypes) {
            $http.get("resourceBuilder/allResources.json").then(
              ...
              function (err) {
                alert(
                  "Error loading allResources.json\n" + angular.toJson(err),
                );
                deferred.reject();
              },
            );
        ...
        parseProfile: function (profile) {
          ...
          if (profile && profile.snapshot && profile.snapshot.element) {
            ...
            profile.snapshot.element.forEach(function (element) {
              ...
              if (arPath[arPath.length - 1] == "extension") {
                if (element.type[0].profile) {
                  ...
                  var urlToExtensionDefinition = Utilities.getProfileFromType(
                    element.type[0],
                  );
                  ...
                  if (!extensionObj[urlToExtensionDefinition]) {
                    ...
                    extensionQuery.push(
                      GetDataFromServer.findResourceByUrlPromise(
                        "StructureDefinition",
                        urlToExtensionDefinition,
            ...
            $q.all(extensionQuery).then(function () {
              //return;
                ...
                if (!ignore) {
                  ...
                  if (arPath[arPath.length - 1] == "extension") {
                    ...
                    if (element.type && element.type[0].profile) {
                      ...
                      var urlToExtensionDefinition =
                        Utilities.getProfileFromType(element.type[0]);
                      //var urlToExtensionDefinition = element.type[0].profile[0];
                      ...
                      if (extensionDefinition) {
                        ...
                        var analysis =
                          Utilities.analyseExtensionDefinition(
                            extensionDefinition,
                          );
                        ...
                        if (!analysis.complexExtension) {
                          ...
                          var newEl;
                          var key =
                            analysis.display || analysis.name || "No Name";
                          ...
                          for (
                            var i = 0;
                            i < extensionDefinition.snapshot.element.length;
                            i++
                          ) {
                          ...
                          if (newEl) {
                            //from the value[x] element in the profile
                            //a simple extension is against the resource root
                            newEl.myMeta = {
                              isExtension: true,
                              extensionType: "simple",
                            };
                            ...
                            //
                            newEl.extensionUrl = Utilities.getProfileFromType(
                              element.type[0],
                            ); //save the url...
                          ...
                          //otherwise,
                          extensionPath =
                            extensionPath.replace(".extension", "") + "." + key;
                          ...
                          analysis.complexExtension.contents.forEach(
                            function (content) {
                              ...
                              if (content.boundValueSet) {
                                childElement.binding = {
                                  valueSetReference: {
                                    reference: content.boundValueSet,
                                  },
                                };
                              }
          ...
          if (SD && SD.snapshot && SD.snapshot.element) {
            ...
            SD.snapshot.element.forEach(function (element) {
              ...
              if (element.value) {
                ...
                if (angular.isArray(element.value)) {
                  if (debug) {
                    console.log(element.path, "array", ar.length);
                  }
                  ...
                  switch (ar.length) {
                      ...
                      element.value.forEach(function (set) {
                        ...
                        if (!isEmpty(set)) {
                          ...
                          if (element.myMeta && element.myMeta.isExtension) {
                            ...
                            var complexExtension = createComplexExtension(
                              element,
                              set,
                            );
                            /*
                    ...
                    default:
                      alert(
                        "Sorry! This resource has a structure I cannot process at path " +
                          path +
                          " , and your data will not be saved. Please contact the author.",
                      );
                      break;
                  }
                } else {
                  ...
                  if (debug) {
                    console.log(element.path, "object", ar.length);
                  }
                  switch (ar.length) {
                    case 1:
            ...
            if (patient) {
              for (var i = 0; i < SD.snapshot.element.length - 1; i++) {
                ...
                if (element.path) {
                  var ar = element.path.split(".");
                  if (ar[1] == "patient" && !resource["patient"]) {
                    //always attached to the root...
                  ...
                  if (ar[1] == "subject" && !resource["subject"]) {
                    //always attached to the root...
                    var reference = { reference: "Patient/" + patient.id };
                    reference.display =
                      ResourceUtilsSvc.getOneLineSummaryOfResource(patient);
            ...
            if (enteredText) {
              ...
              resource["text"] = {
                status: "additional",
                div:
                  "<div><div id='userEntered'>" +
                  htmlEscape(enteredText) +
                  "</div>" +
                  text +
                  "</div>",
              };
        ...
        getResourcesSelectListOfType: function (
          allResources,
          resourceType,
          profileUrl,
        ) {
          ...
          bundle.entry.forEach(function (entry) {
            ...
            if (profileUrl) {
              ...
              if (entry.resource.meta && entry.resource.meta.profile) {
                //if the resource has a profile declaration
                var profiles = entry.resource.meta.profile;
                for (var i = 0; i < profiles.length; i++) {
                  //iterate through each profile on the resource
        ...
        getUniqueResources: function (allResources) {
          ...
          angular.forEach(allResources, function (bundle, type) {
            if (bundle.entry) {
              ar.push({
                key: type,
                display: type + " (" + bundle.entry.length + ")",
              });
            }
          });
...
*/
        ...
        populateTimingList: function () {
          ...
          lst.push({
            ...
            timing: { freq: 2, period: 1, periodUnits: "d" },
          });
          lst.push({
            description: "Three times a day",
            timing: { freq: 3, period: 1, periodUnits: "d" },
          });

          lst.push({
            description: "Every 8 hours",
            timing: { freq: 1, period: 8, periodUnits: "h" },
          });
          lst.push({
            description: "Every 7 days",
            timing: { freq: 1, period: 7, periodUnits: "d" },
          });
          lst.push({
            description: "3-4 times a day",
            timing: { freq: 3, freqMax: 4, period: 1, periodUnits: "d" },
          });
          lst.push({
            description: "Every 4-6 hours",
            timing: { freq: 1, periodMax: 6, period: 1, periodUnits: "h" },
          });
          lst.push({
            description: "Every 21 days for 1 hour",
            timing: {
              duration: 1,
              units: "h",
              freq: 1,
  ...
  .service("ResourceUtilsSvc", function (appConfigSvc) {
    function getPeriodSummary(data) {
      ...
      var txt = "";
      if (data.start) {
        txt += moment(data.start).format("YYYY-MM-DD") + " ";
      }
      if (data.end) {
        txt += "-> " + moment(data.end).format("YYYY-MM-DD");
      }
      return txt;
    }
      ...
      function getString(ar) {
        ...
        if (ar) {
          if (angular.isArray(ar)) {
            //make sure it's an array. eh humanname isn't...
      ...
      getTextSummaryOfDataType: function (dt, value) {
        ...
        switch (dt) {
          ...
          case "CodeableConcept":
            if (value.text) {
              vo.summary = value.text;
              vo.detail = value.text;
            }
            if (value.coding) {
      ...
      getOneLineSummaryOfResource: function (resource, extend) {
        ...
        if (resource) {
          switch (resource.resourceType) {
            ...
            case "CodeSystem":
              return (
                resource.name || resource.description || resource.resourceType
              );
            case "MedicationStatement":
              var txt = "";
              if (resource.medicationCodeableConcept) {
...
*/
            ...
            case "Observation":
              ...
              if (resource.effectiveDateTime) {
                summary =
                  resource.effectiveDateTime.substr(0, 10) + " " + summary;
              }
