...
angular
  ...
  .directive(
    "vsBrowser",
    function (Utilities, GetDataFromServer, $uibModal, $http, appConfigSvc) {
      return {
        restrict: "E",
        scope: {
          trigger: "=",
              ...
              .open({
                ...
                controller: function (
                  ...
                  $http.get(url).then(
                    function (data) {
                      ...
                      if (data.data && data.data.entry) {
                        ...
                        let ext = Utilities.getSingleExtensionValue(
                          $scope.valueSet,
                          authorUrl,
                        );
                        ...
                        $scope.footerMsg =
                          "This ValueSet was not found on the Terminology server";
                        $scope.notFound = true;
                        ...
                        Utilities.addExtensionOnce($scope.valueSet, authorUrl, {
                          valueBoolean: true,
                        });
                  ...
                  $scope.addNewCode = function (code, display) {
                    ...
                    $scope.valueSet.compose.include =
                      $scope.valueSet.compose.include || [];
                    let includeToUse;
                  ...
                  $scope.saveVS = function () {
                    if ($scope.valueSet.id) {
                      ...
                      $http.put(url, $scope.valueSet).then(
                        ...
                        function (err) {
                          alert(
                            "Error saving ValueSet:" + angular.toJson(err.data),
                          );
                        },
                      );
                    } else {
                      ...
                      $http.post(url, $scope.valueSet).then(
                        function (data) {
                          alert(
                            "ValueSet was created. The dialog needs to close.",
                          );
                          $scope.$close();
                  ...
                  $scope.close = function () {
                    if ($scope.isDirty) {
                      if (
                        confirm(
                          "There are unsaved changes. Are you sure you wish to close without saving?",
                        )
                      ) {
                        $scope.$dismiss();
                      }
                  ...
                  $scope.expand = function () {
                    ...
                    if ($scope.vsUrl) {
                      ...
                      if ($scope.vsUrl.indexOf("http://snomed.info") > -1) {
                        ...
                        url =
                          "http://its.patientsfirst.org.nz/RestService.svc/Terminz/ValueSet/$expand?url=";
                        url +=
                      ...
                      $http
                        ...
                        .then(
                          function (data) {
                            ...
                            if (expandedVs.expansion) {
                              ...
                              if (!expandedVs.expansion.contains) {
                                alert(
                                  "The expansion worked fine, but no expanded data was returned",
                                );
                              }
                            } else {
                          ...
                          function (err) {
                            ...
                            if (err.status == 422) {
                              alert(
                                "There were too many concepts to expand - use a filter.",
                              );
                            } else if (err.status == 404) {
                              alert(
                                "Sorry, the ValueSet was not found on the Terminology server so could not be expanded.",
                              );
                            } else {
                              alert(
                                "Sorry, there was an error performing the expansion: " +
                                  angular.toJson(err),
                              );
                            }
                          },
                    ...
                    if (!$scope.selectedvs) {
                      alert(
                        "You must select a ValueSet before the expansion will work!",
                      );
                      return;
                    }
                    ...
                    if (!filter) {
                      ...
                      $scope.query =
                        "ValueSet/" + $scope.selectedvs.id + "/$expand";
                      ...
                      $scope.query =
                        ...
                        "/$expand?filter=" +
                        filter;
                      GetDataFromServer.getFilteredValueSet(
                        $scope.selectedvs.id,
                        filter,
                      ).then(
                        function (data1) {
                        ...
                        function (err) {
                          ...
                          if (err.status == 422) {
                            alert(
                              "There were too many concepts to expand - make the filter more restrictive.",
                            );
                          } else {
                            alert(
                              "Sorry, there was an error performing the expansion: " +
