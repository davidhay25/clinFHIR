...
class FSHImporter extends FSHVisitor_1.default {
        ...
        rawFSHes.forEach(rawFSH => {
            ...
            ctx.entity().forEach(e => {
                ...
                if (e.alias()) {
                    ...
                    let value = (_a = e.alias().SEQUENCE()) === null || _a === void 0 ? void 0 : _a.getText();
                    ...
                    if (name.includes('|')) {
                        ...
                        FSHLogger_1.logger.warn(`Alias ${name} includes unsupported characters. Alias names can only contain letters, numbers, underscores ("_"), hyphens ("-"), and dots (".").`, { file: (_c = doc.file) !== null && _c !== void 0 ? _c : '', location: this.extractStartStop(e.alias()) });
                    }
                    if (this.allAliases.has(name) && this.allAliases.get(name) !== value) {
                        FSHLogger_1.logger.error(`Alias ${name} cannot be redefined to ${value}; it is already defined as ${this.allAliases.get(name)}.`, { file: (_d = doc.file) !== null && _d !== void 0 ? _d : '', location: this.extractStartStop(e.alias()) });
    ...
    parseInstance(instance, location, metaCtx = [], ruleCtx = []) {
        const seenPairs = new Map();
        metaCtx
        ...
        if (common_1.CONFORMANCE_AND_TERMINOLOGY_RESOURCES.has(instance.instanceOf) &&
            !metaCtx.some(ctx => ctx.usage())) {
            FSHLogger_1.logger.warn(`No usage was specified on ${instance.name}. The default #example usage will be applied, but ${instance.instanceOf} Instances are typically definitions. Specify a usage to remove this warning.`, {
                file: this.currentFile,
                location
            });
        }
    ...
    visitVsRule(ctx) {
        ...
        if (ctx.caretValueRule()) {
            ...
            if (rule.pathArray.length > 1) {
                FSHLogger_1.logger.error('Only one concept may be listed before a caret rule on a ValueSet.', rule.sourceInfo);
            }
            ...
            if (rule.pathArray.length > 1) {
                FSHLogger_1.logger.error('Only one concept may be listed before an insert rule on a ValueSet.', rule.sourceInfo);
            }
            else {
                return rule;
            }
    ...
    getArrayPathWithContext(pathArray, parentCtx, isPathRule = false, isInstanceRule = false, suppressError = false) {
    ...
    visitCodeCaretValueRule(ctx, keepSystem = false) {
        var _a, _b, _c, _d;
        const localCodePath = ctx.CODE()
            ? ctx.CODE().map(code => {
                ...
                if (keepSystem) {
                    return `${(_a = parsedCode.system) !== null && _a !== void 0 ? _a : ''}#${parsedCode.code}`;
                }
                else {
        ...
        // Get the caret path, but slice off the starting ^
        caretRule.caretPath = (_a = this.visitCaretPath(ctx.caretPath())) === null || _a === void 0 ? void 0 : _a.slice(1);
        ...
        if ((_b = ctx.value()) === null || _b === void 0 ? void 0 : _b.NUMBER()) {
            caretRule.rawValue = ctx.value().NUMBER().getText();
        }
        if ((_c = ctx.value()) === null || _c === void 0 ? void 0 : _c.bool()) {
            caretRule.rawValue = ctx.value().bool().getText();
        }
        caretRule.isInstance =
            ((_d = ctx.value()) === null || _d === void 0 ? void 0 : _d.name()) != null && !this.allAliases.has(ctx.value().name().getText());
        return caretRule;
    }
    visitObeysRule(ctx) {
        const rules = [];
    ...
    // when parsing a ValueSet, we need to keep the system.
    // in all other cases, the system is not needed.
    visitCodeInsertRule(ctx, keepSystem = false) {
    ...
    visitVsComponent(ctx) {
        ...
        if (ctx.vsConceptComponent()) {
            ...
            if (vsComponent.concepts.length === 1) {
                ...
                this.getArrayPathWithContext([`${vsComponent.concepts[0].system}#${vsComponent.concepts[0].code}`], ctx, false, false, true);
            }
            else {
                // reset the context
     ...
     */
    prependPathContext(path, parentCtx, isPathRule, isInstanceRule, suppressError) {
        try {
    ...
    extractStartStop(ctx, suppressError = false) {
        ...
        else if (ctx instanceof antlr4_1.TerminalNode) {
            return {
