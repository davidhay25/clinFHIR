...
class CodeSystemExporter {
    ...
    setCaretPathRules(codeSystem, rules, codeSystemSD) {
        ...
        const successfulRulesWithInstances = successfulRules
            ...
            try {
                ...
                const { pathParts } = codeSystemSD.validateValueAtPath(path, replacedRule.value, this.fisher, inlineResourceTypes);
                ...
                if (originalErr instanceof errors_1.MismatchedTypeError &&
                    ...
                    const instanceExporter = new _1.InstanceExporter(this.tank, this.pkg, this.fisher);
                    const { instance, pathParts } = (0, common_1.validateInstanceFromRawValue)(codeSystem, rule, instanceExporter, this.fisher, originalErr, inlineResourceTypes);
        ...
        (0, common_1.setImpliedPropertiesOnInstance)(codeSystem, codeSystemSD, [...ruleMap.keys()], inlineResourcePaths.map(i => i.caretPath), this.fisher, knownSlices);
        for (const { rule, inlineResourceTypes } of successfulRulesWithInstances) {
            try {
                (0, common_1.setPropertyOnDefinitionInstance)(codeSystem, rule.path.length > 1 ? `${rule.path}.${rule.caretPath}` : rule.caretPath, rule.value, this.fisher, inlineResourceTypes);
            }
            catch (err) {
    ...
    exportCodeSystem(fshDefinition) {
        ...
        this.setCaretPathRules(codeSystem, fshDefinition.rules.filter(rule => rule instanceof rules_1.CaretValueRule), codeSystemSD);
        ...
        (0, common_1.checkForMultipleChoice)(fshDefinition, codeSystem, codeSystemSD);
