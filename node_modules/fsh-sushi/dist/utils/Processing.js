...
function readConfig(input) {
    ...
    if (!config.fhirVersion.some(v => isSupportedFHIRVersion(v))) {
        FSHLogger_1.logger.error(`The ${path_1.default.basename(config.filePath)} must specify a supported version of FHIR. Found ${config.fhirVersion}. Be sure to` +
...
async function init(name = null, options = {}) {
    ...
    else if (!readline_sync_1.default.keyInYN(`SUSHI project will be created in ${outputDir}. Proceed?`)) {
