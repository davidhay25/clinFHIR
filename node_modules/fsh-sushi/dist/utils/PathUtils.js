...
exports.collectValuesAtElementIdOrPath = collectValuesAtElementIdOrPath;
 ...
 */
function parseFSHPath(fshPath, sourceInfo) {
    var _a, _b;
    const pathParts = [];
    const seenSlices = [];
    const indexRegex = /^[0-9]+$/;
    const splitPath = fshPath === '.' ? [fshPath] : (0, common_1.splitOnPathPeriods)(fshPath);
    for (const pathPart of splitPath) {
        const parsedPart = {
            base: (_b = (_a = pathPart.match(/^([^\[]+(\[x\])?)/)) === null || _a === void 0 ? void 0 : _a[0]) !== null && _b !== void 0 ? _b : ''
        };
        if (pathPart.length > parsedPart.base.length) {
            ...
            parsedPart.brackets = Array.from(pathPart.slice(parsedPart.base.length).matchAll(/\[([^\[\]]|\[(?:[^\[\]]*)\])*\]/g)).map(match => match[0].slice(1, -1));
            seenSlices.push(...parsedPart.brackets.filter(b => !indexRegex.test(b) && !(b === '+' || b === '=')));
            if (seenSlices.length > 0) {
                parsedPart.slices = [...seenSlices];
            }
            const parsedPartLength = parsedPart.base.length +
                parsedPart.brackets
                    .map(b => b.length + 2)
            ...
            if (pathPart.length !== parsedPartLength) {
                ...
                if (sourceInfo) {
                    FSHLogger_1.logger.error(message, sourceInfo);
                }
                else {
 ...
 */
function convertSoftIndices(element, pathMap) {
 ...
 */
function convertSoftIndicesStrict(element, pathMap, maxPathMap) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    // Must account for a pathPart's base name, prior portions of the path, as well as any slices it's contained in.
    const mapName = `${(_a = element.prefix) !== null && _a !== void 0 ? _a : ''}.${element.base}|${((_b = element.slices) !== null && _b !== void 0 ? _b : []).join('|')}`;
    const indexRegex = /^[0-9]+$/;
    let addToBaseElement; // track the amount of indices we need to add to the base element of a slice
    if (!pathMap.has(mapName)) {
        const existingNumericBracket = (_c = element.brackets) === null || _c === void 0 ? void 0 : _c.find(bracket => indexRegex.test(bracket));
        if (existingNumericBracket) {
    ...
    if (((_g = element.slices) === null || _g === void 0 ? void 0 : _g.length) > 0 && addToBaseElement != null) {
        for (let takeSlices = element.slices.length - 1; takeSlices >= 0; takeSlices--) {
            const lessSlicedMapName = `${(_h = element.prefix) !== null && _h !== void 0 ? _h : ''}.${element.base}|${element.slices
                .slice(0, takeSlices)
                .join('|')}`;
 ...
 */
function resolveSoftIndexing(rules, strict = false) {
    const pathMap = new Map(); // tracks the current index at a path
    const maxPathMap = new Map(); // tracks the maximum index seen at a path
    const caretPathMap = new Map();
    const maxCaretPathMap = new Map();
 ...
 */
function collectValuesAtElementIdOrPath(idOrPath, object) {
    const ignoredSliceRequirements = [];
    let values = [object];
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        values = (0, lodash_1.flatten)(newResults);
    }
