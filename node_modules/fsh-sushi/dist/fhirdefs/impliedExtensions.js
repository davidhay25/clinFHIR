...
exports.IMPLIED_EXTENSION_REGEX = void 0;
exports.isImpliedExtension = isImpliedExtension;
exports.materializeImpliedExtension = materializeImpliedExtension;
exports.materializeImpliedExtensionMetadata = materializeImpliedExtensionMetadata;
const lodash_1 = require("lodash");
const utils_1 = require("../utils");
const fhirtypes_1 = require("../fhirtypes");
const common_1 = require("../fhirtypes/common");
exports.IMPLIED_EXTENSION_REGEX = /^http:\/\/hl7\.org\/fhir\/([1345]\.0)\/StructureDefinition\/extension-(([^./]+)\..+)$/;
// This map represents the relationship from the version part of the extension URL to the FHIR package
const VERSION_TO_PACKAGE_MAP = {
...
const RESOURCE_TO_RESOURCE_MAP = {
    // Mappings R2 -> R4/R5
 ...
 */
function isImpliedExtension(url) {
    return exports.IMPLIED_EXTENSION_REGEX.test(url);
}
/**
 ...
 */
function materializeImpliedExtension(url, defs) {
    var _a;
    const match = decodeURI(url).match(exports.IMPLIED_EXTENSION_REGEX);
    if (match == null) {
        utils_1.logger.error(`Unsupported extension URL: ${url}. Extension URLs for converting between versions of ` +
    ...
    if (supplementalDefs == null) {
        ...
        const fhirVersion = (_a = defs.fishForFHIR('StructureDefinition', utils_1.Type.Resource)) === null || _a === void 0 ? void 0 : _a.fhirVersion;
        utils_1.logger.error(`The extension ${url} requires the following dependency to be declared in your sushi-config.yaml ` +
            `file:\n  hl7.fhir.extensions.${extRelease}: ${fhirVersion}`);
        return;
    }
    const sd = supplementalDefs.fishForFHIR(type, utils_1.Type.Resource, utils_1.Type.Type);
    if (sd == null) {
        utils_1.logger.error(`Cannot process extension (${url}) since ${type} is not a valid resource or ` +
            `data type in ${supplementalPackage}.`);
        return;
    }
    // NOTE: DSTU2 definitions don't populate element id, so fallback to path in that case
    const ed = sd.snapshot.element.find((ed) => { var _a; return ((_a = ed.id) !== null && _a !== void 0 ? _a : ed.path) === id; });
    if (ed == null) {
        utils_1.logger.error(`Cannot process extension (${url}) since ${id} is not a valid id in ${type}.`);
        return;
    }
    else if (isUnsupported(ed)) {
        utils_1.logger.error(`Cannot process extension (${url}) since its type is Resource. ` +
            'See: http://hl7.org/fhir/versions.html#extensions.');
        return;
    }
    const ext = fhirtypes_1.StructureDefinition.fromJSON(defs.fishForFHIR('Extension'));
...
function materializeImpliedExtensionMetadata(url, defs) {
    var _a, _b, _c, _d;
 ...
 */
function isUnsupported(fromED) {
    var _a;
    return (_a = fromED.type) === null || _a === void 0 ? void 0 : _a.some((t) => t.code === 'Resource');
}
/**
 ...
 */
function isComplex(fromED, defs) {
    var _a;
    const codes = (0, lodash_1.union)((_a = fromED.type) === null || _a === void 0 ? void 0 : _a.map((t) => t.code));
    if (codes.length !== 1) {
    ...
    const def = defs.fishForFHIR(codes[0], utils_1.Type.Resource, utils_1.Type.Type);
 ...
 */
function applyMetadataToExtension(fromSD, fromED, fromVersion, toExt) {
    var _a;
    const elementId = (_a = fromED.id) !== null && _a !== void 0 ? _a : fromED.path;
 ...
 */
function applyMetadataToElement(fromED, toED) {
    var _a;
    ...
    if (fromED.comment || fromED.comments) {
        toED.comment = (_a = fromED.comment) !== null && _a !== void 0 ? _a : fromED.comments;
    }
 ...
 */
function applyContent(fromSD, fromED, fromVersion, toExt, toBaseED, visitedTypes, defs, supplementalDefs) {
    var _a;
    const url = (_a = toBaseED.sliceName) !== null && _a !== void 0 ? _a : toExt.url;
    ...
    if (isComplex(fromED, defs)) {
        applyToExtensionElement(fromSD, fromED, fromVersion, toExt, extensionED, visitedTypes.slice(0), // pass down a clone so that siblings aren't adding to the same array (we only care recursive descendents)
        defs, supplementalDefs);
 ...
 * @param defs {FHIRDefinitions} - the primary FHIRDefinitions object for the current project
 */
function applyToValueXElement(fromED, fromVersion, toExt, toED, defs) {
    var _a, _b, _c;
    toED.type = getTypes(fromED, fromVersion);
    if (fromED.binding) {
        toED.bindToVS(
        // Handle different representation in DSTU2 and STU3
        (_b = (_a = fromED.binding.valueSet) !== null && _a !== void 0 ? _a : fromED.binding.valueSetUri) !== null && _b !== void 0 ? _b : (_c = fromED.binding.valueSetReference) === null || _c === void 0 ? void 0 : _c.reference, fromED.binding.strength);
 ...
 *   version of FHIR corresponding to the source element represented by the implied extension.
 */
function applyToExtensionElement(fromSD, fromED, fromVersion, toExt, toED, visitedTypes, defs, supplementalDefs) {
    var _a, _b, _c, _d, _e;
    let edId = (_a = fromED.id) !== null && _a !== void 0 ? _a : fromED.path;
    // First look for children directly in the SD
    let childElements = fromSD.snapshot.element.filter((e) => { var _a; return ((_a = e.id) !== null && _a !== void 0 ? _a : e.path).startsWith(`${edId}.`); });
    ...
    if (childElements.length === 0) {
        ...
        if (edTypes.length === 1) {
            const typeSD = supplementalDefs.fishForFHIR(edTypes[0].code, utils_1.Type.Resource, utils_1.Type.Type);
            if (typeSD) {
                if (visitedTypes.indexOf(typeSD.url) !== -1) {
                    ...
                    utils_1.logger.warn(`Definition of extension (${toExt.url}) is incomplete because ${parentId} causes ` +
                ...
                edId = (_b = typeSD.id) !== null && _b !== void 0 ? _b : typeSD.path;
    ...
    childElements.forEach((e) => {
        var _a;
        const id = (_a = e.id) !== null && _a !== void 0 ? _a : e.path;
        ...
        if (tail.indexOf('.') === -1 && !IGNORED_CHILDREN.includes(tail)) {
            ...
            slice.type = [new fhirtypes_1.ElementDefinitionType('Extension')];
            slice.unfold(defs);
            applyContent(fromSD, e, fromVersion, toExt, slice, visitedTypes.slice(0), // pass down a clone so that siblings aren't adding to the same array (we only care recursive descendents)
            defs, supplementalDefs);
        }
    });
    ...
    if (((_d = (_c = fromED.type) === null || _c === void 0 ? void 0 : _c[0]) === null || _d === void 0 ? void 0 : _d.code) === 'CodeableReference') {
        if (fromED.binding) {
        ...
        if (((_e = fromED.type[0].targetProfile) === null || _e === void 0 ? void 0 : _e.length) > 0) {
 ...
 */
function getTypes(fromED, fromVersion) {
    var _a, _b, _c, _d, _e, _f;
    if (fromED.type == null || fromED.type.length === 0) {
        return [];
    }
    else if (fromVersion === '4.0' || fromVersion === '5.0') {
        return fromED.type.map(fhirtypes_1.ElementDefinitionType.fromJSON);
    }
        ...
        const r4types = [];
        for (const r3Type of fromED.type) {
            let r4Type = r4types.find(t => t.code === r3Type.code);
            if (r4Type == null) {
                r4Type = new fhirtypes_1.ElementDefinitionType(r3Type.code);
                r4types.push(r4Type);
            }
            if (r3Type.profile != null) {
                r4Type.profile = (0, lodash_1.union)((_a = r4Type.profile) !== null && _a !== void 0 ? _a : [], [r3Type.profile]);
            }
            if (r3Type.targetProfile != null) {
                r4Type.targetProfile = (0, lodash_1.union)((_b = r4Type.targetProfile) !== null && _b !== void 0 ? _b : [], [r3Type.targetProfile]);
            }
            if (r3Type.aggregation != null) {
                r4Type.aggregation = (0, lodash_1.union)((_c = r4Type.aggregation) !== null && _c !== void 0 ? _c : [], r3Type.aggregation);
            }
        ...
        const r4types = [];
        for (const r2Type of fromED.type) {
            let r4Type = r4types.find(t => t.code === r2Type.code);
            if (r4Type == null) {
                r4Type = new fhirtypes_1.ElementDefinitionType(r2Type.code);
                r4types.push(r4Type);
            }
            if (r2Type.profile != null) {
                if (r2Type.code === 'Reference') {
                    r4Type.targetProfile = (0, lodash_1.union)((_d = r4Type.targetProfile) !== null && _d !== void 0 ? _d : [], r2Type.profile);
                }
                else {
                    r4Type.profile = (0, lodash_1.union)((_e = r4Type.profile) !== null && _e !== void 0 ? _e : [], r2Type.profile);
                }
            }
            if (r2Type.aggregation != null) {
                r4Type.aggregation = (0, lodash_1.union)((_f = r4Type.aggregation) !== null && _f !== void 0 ? _f : [], r2Type.aggregation);
            }
        }
        return r4types;
    }
 ...
 * @param defs {FHIRDefinitions} - the primary FHIRDefinitions object for the current project
 */
function filterAndFixTypes(toExt, toED, defs) {
    var _a;
    const unsupportedTypes = [];
    toED.type = (_a = toED.type) === null || _a === void 0 ? void 0 : _a.filter(t => {
        var _a, _b, _c, _d;
        ...
        if (defs.fishForFHIR(t.code, utils_1.Type.Resource, utils_1.Type.Type) == null) {
        ...
        t.profile = (_a = t.profile) === null || _a === void 0 ? void 0 : _a.filter(p => {
            if (defs.fishForFHIR(p, utils_1.Type.Profile, utils_1.Type.Extension) == null) {
        ...
        if (((_b = t.profile) === null || _b === void 0 ? void 0 : _b.length) === 0) {
            delete t.profile;
        }
        ...
        t.targetProfile = (_c = t.targetProfile) === null || _c === void 0 ? void 0 : _c.map(p => {
            if (defs.fishForFHIR(p, utils_1.Type.Resource, utils_1.Type.Profile)) {
                return p;
            }
        ...
        if (((_d = t.targetProfile) === null || _d === void 0 ? void 0 : _d.length) === 0) {
    ...
    if (unsupportedTypes.length > 0) {
        ...
        utils_1.logger.warn(`Definition of extension (${toExt.url}) is incomplete since the following ${typesHave} no ` +
            `equivalent in FHIR ${toExt.fhirVersion}: ${unsupportedTypes.join(', ')}.`);
    }
}
//# sourceMappingURL=impliedExtensions.js.map