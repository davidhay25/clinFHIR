...
class ConceptRule extends Rule_1.Rule {
    ...
    toFSH() {
        const quotedCodes = [...this.hierarchy, this.code].map(code => /\s/.test(code) ? `#"${code}"` : `#${code}`);
        let line = `* ${quotedCodes.join(' ')}`;
        if (this.display) {
            line += ` "${common_1.fshifyString(this.display)}"`;
        }
        if (this.definition) {
            ...
            line += ` ${quotes}${common_1.fshifyString(this.definition)}${quotes}`;
        }
        return line;
    }
}
