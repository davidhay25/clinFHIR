...
class ElementDefinition {
        ...
        for (let prop of PROPS) {
            ...
            if (prop && !lodash_1.isEqual(this[prop], original[prop])) {
                if (ADDITIVE_PROPS.includes(prop)) {
                    // @ts-ignore
                    diff[prop] = lodash_1.differenceWith(this[prop], original[prop], lodash_1.isEqual);
                    // @ts-ignore
                    if (lodash_1.isEmpty(diff[prop])) {
                        // @ts-ignore
                        delete diff[prop];
                    }
                }
    ...
    applyAddElementRule(rule, fisher) {
        ...
        if (!lodash_1.isEmpty(rule.short)) {
            this.short = rule.short;
        }
        if (lodash_1.isEmpty(rule.definition)) {
            if (!lodash_1.isEmpty(rule.short)) {
    ...
    initializeElementType(rule, fisher) {
        ...
        const finalTypes = lodash_1.uniqWith(initialTypes, lodash_1.isEqual);
        if (refTypeCnt > 0) {
    ...
    findTypeMatch(type, targetTypes, fisher) {
        ...
        for (const md of lineage) {
            if (type.isReference) {
                ...
                matchedType = targetTypes.find(t2 => common_1.isReferenceType(t2.code) &&
                ...
                matchedType = targetTypes.find(t2 => {
                    ...
                    const matchesProfile = this.getTypeLineage(md.sdType, fisher).some(ancestor => ancestor.sdType === t2.code) && ((_a = t2.profile) === null || _a === void 0 ? void 0 : _a.includes(md.url));
    ...
    applyProfiles(newType, targetType, matches) {
        ...
        for (const match of matches) {
            ...
            else if (common_1.isReferenceType(match.code) && !common_1.isReferenceType(match.metadata.sdType)) {
    ...
    applyTypeIntersection(type, targetType, matches) {
        ...
        for (const match of matches) {
            ...
            const typeString = common_1.isReferenceType(match.code) || match.code === 'canonical'
                ? match.code
    ...
    assignValue(value, exactly = false, fisher) {
        ...
        switch (type) {
            ...
            case 'Canonical':
                ...
                let canonicalUrl;
                if (!this.typeSatisfiesTargetProfile(canonicalDefinition === null || canonicalDefinition === void 0 ? void 0 : canonicalDefinition.resourceType, fisher)) {
            ...
            case 'InstanceDefinition':
                ...
                this.assignFHIRValue(stringVal, value.toJSON(), exactly, (_d = value._instanceMeta.sdType) !== null && _d !== void 0 ? _d : value.resourceType);
                break;
            default:
     ...
     */
    typeSatisfiesTargetProfile(sdType, fisher) {
        ...
        if (sdType && this.type[0].targetProfile) {
            ...
            this.type[0].targetProfile.forEach(tp => {
                ...
                const tpType = (_a = fisher.fishForMetadata(unversionedProfile)) === null || _a === void 0 ? void 0 : _a.sdType;
                if (tpType) {
    ...
    assignFshCode(code, exactly = false, fisher) {
        ...
        if (code.system) {
            ...
            const vsURI = (_b = (_a = fisher === null || fisher === void 0 ? void 0 : fisher.fishForMetadata(code.system, utils_1.Type.ValueSet)) === null || _a === void 0 ? void 0 : _a.url) !== null && _b !== void 0 ? _b : '';
            if (vsURI) {
                throw new errors_1.MismatchedBindingTypeError(code.system, this.path, 'CodeSystem');
            }
            else if (!valid_url_1.isUri(csURI)) {
        ...
        if (proceedToUnfold) {
            ...
            if (newElements.length === 0) {
                ...
                if (json) {
                    const def = StructureDefinition_1.StructureDefinition.fromJSON(json);
                    if (def.inProgress) {
                        utils_1.logger.debug(`Warning: Circular relationship detected between ${(_d = this.structDef) === null || _d === void 0 ? void 0 : _d.name} and ${def.name}. As a result, it is possible that the definition of ${(_e = this.structDef) === null || _e === void 0 ? void 0 : _e.name} may be based on incomplete components of ${def.name}.`);
                    }
