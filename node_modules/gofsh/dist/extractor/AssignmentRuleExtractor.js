...
class AssignmentRuleExtractor {
    static process(input, entityName) {
        ...
        if (matchingKey) {
            ...
            const assignmentRule = new exportable_1.ExportableAssignmentRule((0, utils_1.getPath)(input));
            ...
            if (isPrimitiveValue(matchingValue)) {
                ...
                if (typeof matchingValue === 'string') {
                    if (matchingKey.endsWith('Code')) {
                        ...
                        if (matchingKey.endsWith('DateTime')) {
                            if (!utils_1.dateTimeRegex.test(matchingValue)) {
                                utils_1.logger.warn(`Value ${matchingValue} on ${entityName} element ${assignmentRule.path} is not a valid FHIR dateTime`);
                            }
                        }
                        else if (matchingKey.endsWith('Date')) {
                            if (!utils_1.dateRegex.test(matchingValue)) {
                                utils_1.logger.warn(`Value ${matchingValue} on ${entityName} element ${assignmentRule.path} is not a valid FHIR date`);
                            }
                        }
                        else if (matchingKey.endsWith('Time')) {
                            if (!utils_1.timeRegex.test(matchingValue)) {
                                utils_1.logger.warn(`Value ${matchingValue} on ${entityName} element ${assignmentRule.path} is not a valid FHIR time`);
                            }
                        }
                        else if (matchingKey.endsWith('Instant')) {
                            if (!utils_1.instantRegex.test(matchingValue)) {
                                utils_1.logger.warn(`Value ${matchingValue} on ${entityName} element ${assignmentRule.path} is not a valid FHIR instant`);
                            }
                        }
                    }
                }
                ...
                if (matchingKey.endsWith('Coding') && isCoding(matchingValue)) {
                    assignmentRule.value = new fsh_sushi_1.fshtypes.FshCode(matchingValue.code, matchingValue.system, matchingValue.display ? (0, common_1.fshifyString)(matchingValue.display) : undefined);
                    ...
                    assignmentRule.value = new fsh_sushi_1.fshtypes.FshCode(matchingValue.coding[0].code, matchingValue.coding[0].system, matchingValue.coding[0].display
                        ? (0, common_1.fshifyString)(matchingValue.coding[0].display)
                        : undefined);
                    ...
                    assignmentRule.value = new fsh_sushi_1.fshtypes.FshReference(matchingValue.reference, matchingValue.display ? (0, common_1.fshifyString)(matchingValue.display) : undefined);
