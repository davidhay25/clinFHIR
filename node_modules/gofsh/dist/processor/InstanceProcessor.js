...
class InstanceProcessor {
    ...
    static extractRules(input, target, fisher, options) {
        ...
        const inputJSON = (0, lodash_1.cloneDeep)(input);
        ...
        if (instanceOfJSON == null) {
            ...
            if ((0, lodash_1.isEmpty)(inputJSON.meta.profile)) {
                delete inputJSON.meta.profile;
            }
            if ((0, lodash_1.isEmpty)(inputJSON.meta)) {
        ...
        const flatInstanceArray = (0, lodash_1.toPairs)((0, utils_1.getPathValuePairs)(inputJSON));
        flatInstanceArray.forEach(([path], i) => {
            const assignmentRule = new exportable_1.ExportableAssignmentRule(path);
            assignmentRule.value = (0, utils_1.getFSHValue)(i, flatInstanceArray, instanceOfJSON.type, `${target.instanceOf} ${target.id}`, fisher);
            // if the value is empty, we can't use that
            if ((0, utils_1.isFSHValueEmpty)(assignmentRule.value)) {
                utils_1.logger.error(`Value in Instance ${target.name} at path ${path} is empty. No assignment rule will be created.`);
            }
            else {
