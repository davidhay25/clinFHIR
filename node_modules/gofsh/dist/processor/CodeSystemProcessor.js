...
class CodeSystemProcessor {
    ...
    static extractConceptRules(concept, codeSystemName, fisher, hierarchy) {
        ...
        conceptRules.push(newConceptRule, ...extractor_1.CaretValueRuleExtractor.processConcept(concept, [...newConceptRule.hierarchy, concept.code].map(code => `#${code}`), codeSystemName, 'CodeSystem', fisher));
        if (concept.concept) {
            const hierarchyArr = (0, lodash_1.clone)(newConceptRule.hierarchy);
    ...
    static process(input, fisher, config) {
        ...
        if (this.isProcessableCodeSystem(input)) {
            ...
            (0, common_1.makeNameSushiSafe)(codeSystem);
            return codeSystem;
        }
    }
