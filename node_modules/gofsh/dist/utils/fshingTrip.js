...
const _1 = require(".");
function fshingTrip(inDir, outDir, lake, useLocalSUSHI) {
    ...
    try {
        (0, child_process_1.execSync)(`${useLocalSUSHI ? '' : 'npx '}sushi ${outDir}`, { stdio: 'inherit' });
    }
    catch (_a) {
    ...
    const files = (0, lodash_1.union)(Array.from(inputFilesMap.keys()), Array.from(outputFilesMap.keys())).filter(file => ![
        '.index.json',
        'ig-r4.json',
        'package.json',
        'validation-oo.json',
    ...
    files.forEach(file => {
        ...
        // unnecessary diffs
        const originalInputJSON = (0, lodash_1.cloneDeep)(inputFileJSON);
        ...
        const emptyRootDifferential = ((_e = (_d = inputFileJSON.differential) === null || _d === void 0 ? void 0 : _d.element) === null || _e === void 0 ? void 0 : _e.length) &&
            (0, lodash_1.isEqual)(Object.keys(inputFileJSON.differential.element[0]), ['id', 'path']);
        if (emptyRootDifferential) {
            inputFileJSON.differential.element.splice(0, 1);
        }
        // If two files are deeply equal, show no diff
        if ((0, lodash_1.isEqual)(inputFileJSON, outputFileJSON)) {
            return;
        }
        ...
        const patch = (0, diff_1.createTwoFilesPatch)(path_1.default.relative(process.cwd(), inputFilePath), path_1.default.relative(process.cwd(), outputFilePath), JSON.stringify(inputFileJSON, null, 2).replace(/^""$/, ''), JSON.stringify(outputFileJSON, null, 2).replace(/^""$/, ''));
        if (!/@@/.test(patch)) {
            // No difference found
            return;
        }
...
function getFilesMap(dir, lake) {
    const files = (0, _1.getFilesRecursive)(dir).filter(file => file.endsWith('.json'));
    ...
    files.forEach(file => {
        var _a;
        const fileName = (_a = lakeMap.get(file)) !== null && _a !== void 0 ? _a : path_1.default.basename(file);
