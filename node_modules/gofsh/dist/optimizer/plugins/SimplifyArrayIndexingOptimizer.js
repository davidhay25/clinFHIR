    ...
    optimize(pkg) {
        ...
        [
            ...
            const parsedPaths = ruleArr.map((rule) => {
                const parsedPath = {
                    path: fsh_sushi_1.utils.parseFSHPath(rule.path),
                    caretPath: []
                };
                ...
                if (rule instanceof exportable_1.ExportableCaretValueRule) {
                    parsedPath.caretPath = fsh_sushi_1.utils.parseFSHPath(rule.caretPath);
                }
                return parsedPath;
            });
            ...
            parsedPaths.forEach((parsedRule, ruleIndex) => {
                ...
                const key = originalRule instanceof exportable_1.ExportableCaretValueRule && originalRule.isCodeCaretRule
...
function removeZeroIndices(parsedPaths) {
    ...
    caretPathElements.forEach((element) => {
        ...
        const hasOthers = referenceCaretPathElements.some(e => e.base === element.base &&
            e.prefix === element.prefix &&
            e.brackets &&
            !(e.brackets.includes('0') || e.brackets.includes('=')));
        if (!hasOthers) {
