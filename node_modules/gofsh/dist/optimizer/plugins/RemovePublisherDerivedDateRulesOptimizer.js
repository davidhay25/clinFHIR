...
exports.default = {
    ...
    description: 'Remove ^date rules that appear to be the result of the IG Publisher automatically setting a date',
    optimize(pkg) {
        ...
        for (const resource of [
            ...
            for (const rule of resource.rules) {
                if (rule instanceof exportable_1.ExportableCaretValueRule &&
                    rule.caretPath === 'date' &&
                    rule.path === '') {
                    const dateValue = rule.value; // If the rule assigns a date, the value will be a string.
                    if (!date)
                        date = dateValue; // Set the date to match
                    if (date !== dateValue) {
                        allDatesMatch = false;
        ...
        if (allDatesMatch && usesGMT) {
            const DEFAULT_DATE = new exportable_1.ExportableCaretValueRule('');
            ...
            [
                ...
                resource.rules = resource.rules.filter(rule => !(0, lodash_1.isEqual)(rule, DEFAULT_DATE));
            });
        }
    }
};
//# sourceMappingURL=RemovePublisherDerivedDateRulesOptimizer.js.map