...
IG.id = 'cf-artifacts-uscore-3';
IG.description = "US Core, R3";
IG.extension = [{url: "http://clinfhir.com/fhir/StructureDefinition/cfAuthor",valueBoolean:true}]
IG.page = {source:'http://hl7.org/fhir/us/core/',title:'Specification',kind:'page',page:[]};



//var localFileRoot = __dirname;
var localFileRoot = "/Users/davidha/Dropbox/fhirIGs/USCore/";
    ...
    arFiles.forEach(function (fileName) {
        ...
        if (fileName.substr(0,10) == 'CodeSystem') {
            ...
            try {
                ...
                //I can't save a reference to an oid in the hapi server...
                var url = json.url;
                if (url.indexOf('urn:oid') == -1) {
                    var IGEntry = {acronym: 'terminology', description: json.name, sourceReference: {reference: url}};
    ...
    arFiles.forEach(function (fileName) {
        ...
        if (fileName.substr(0,5) == 'Value') {
            ...
            try {
                ...
                //race and ethnocity has oid
                var url = json.url;
               // url = url.replace("urn:oid","http://urn:oid")
